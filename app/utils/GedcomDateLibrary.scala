...
class GedcomDateLibrary @Inject()(
    ...
    def extractDate(gedcomDate: String): Option[LocalDate] = {
        ...
        val republicanDateRegex = """.*\b(VEND|BRUM|FRIM|NIVO|PLUV|VENT|GERM|FLOR|PRAI|MESS|THER|FRUC|COMP|I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|XIII|XIV|XV)\b.*""".r
        
        val removeModifiers = CalendarConstants.keywordsPeriod.map { (key, _) =>
          key -> ""
        } ++ Map("@#[A-Z ]+@".r -> "")

        val trimmedGedcomDate = removeModifiers.foldLeft(gedcomDate) {
        ...
        trimmedGedcomDate match {
            ...
            case _ => {
                val localDate = Try(LocalDate.parse(trimmedGedcomDate, dateFormatter)).map(Some(_))
                  .getOrElse(None)

                if localDate.isEmpty then
                    println(s"Failed to parse date: `$trimmedGedcomDate` ($gedcomDate)")
