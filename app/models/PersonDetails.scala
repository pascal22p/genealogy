  ...
  val mysqlParser: RowParser[PersonDetails] =
    get[Int]("indi_id") ~
      ...
      case id ~ base ~ surname ~ firstname ~ sex ~ timestamp ~ firstnamePrefix ~
        ...
        PersonDetails(base, id, firstname, surname, Sex.fromString(sex), timestamp.getOrElse(Instant.now), firstnamePrefix, surnamePrefix,
          nameSuffix, nameGiven, nameNickname, resn)
    }
}