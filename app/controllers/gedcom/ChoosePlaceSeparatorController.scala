package controllers.gedcom

import java.nio.file.Files
import java.nio.file.Paths
import javax.inject.*

import scala.concurrent.ExecutionContext
import scala.concurrent.Future

import actions.AuthJourney
import config.AppConfig
import models.forms.extensions.FillFormExtension.filledWith
import models.forms.PlacesElementsSeparatorForm
import models.journeyCache.UserAnswersKey.*
import models.AuthenticatedRequest
import play.api.data.Form
import play.api.i18n.*
import play.api.mvc.*
import repositories.JourneyCacheRepository
import utils.Constants
import views.html.gedcom.ChoosePlaceSeparatorView
import services.gedcom.GedcomCommonParser

@Singleton
class ChoosePlaceSeparatorController @Inject() (
    authJourney: AuthJourney,
    appConfig: AppConfig,
    gedcomCommonParser: GedcomCommonParser,
    journeyCacheRepository: JourneyCacheRepository,
    choosePlaceSeparatorView: ChoosePlaceSeparatorView,
    val controllerComponents: ControllerComponents
)(implicit ec: ExecutionContext)
    extends BaseController
    with I18nSupport {

  def showForm: Action[AnyContent] = authJourney.authWithAdminRight.async {
    implicit request: AuthenticatedRequest[AnyContent] =>
      for {
        maybeChooseGedcomFileQuestion <- journeyCacheRepository.get(ChooseGedcomFileQuestion)
        maybePlacesElementsQuestion   <- journeyCacheRepository.get(PlacesElementsSeparatorQuestion)
      } yield {
        (maybeChooseGedcomFileQuestion, maybePlacesElementsQuestion) match {
          case (Some(chooseGedcomFileQuestion), placesElementsQuestion) =>
            val basePath = Paths.get(appConfig.uploadPath)
            val sanitise = s"./${basePath.resolve(chooseGedcomFileQuestion.selectedFile).normalize()}"
            if (sanitise.startsWith(s"$basePath") && Files.exists(Paths.get(sanitise))) {
              val placesSample = gedcomCommonParser.getSamplePlaces(sanitise, Constants.maxSampleSize * 10)
              val form         = PlacesElementsSeparatorForm.form.filledWith(placesElementsQuestion)
              Ok(choosePlaceSeparatorView(form, placesSample))
            } else {
              InternalServerError("Gedcom file path should be valid")
            }

          case _ => Redirect(controllers.gedcom.routes.ImportGedcomController.startJourney)
        }
      }
  }

  def showFormOnSubmit: Action[AnyContent] = authJourney.authWithAdminRight.async {
    implicit request: AuthenticatedRequest[AnyContent] =>
      val errorFunction: Form[PlacesElementsSeparatorForm] => Future[Result] = {
        (formWithErrors: Form[PlacesElementsSeparatorForm]) =>
          journeyCacheRepository.get(ChooseGedcomFileQuestion).map {
            case Some(chooseGedcomFileQuestion) =>
              val basePath = Paths.get(appConfig.uploadPath)
              val sanitise = s"./${basePath.resolve(chooseGedcomFileQuestion.selectedFile).normalize()}"
              if (sanitise.startsWith(s"$basePath") && Files.exists(Paths.get(sanitise))) {
                val placesSample = gedcomCommonParser.getSamplePlaces(sanitise, Constants.maxSampleSize * 10)
                BadRequest(choosePlaceSeparatorView(formWithErrors, placesSample))
              } else {
                InternalServerError("Gedcom file path should be valid")
              }
            case _ => Redirect(controllers.gedcom.routes.ImportGedcomController.startJourney)
          }
      }

      val successFunction: PlacesElementsSeparatorForm => Future[Result] = { (dataForm: PlacesElementsSeparatorForm) =>
        journeyCacheRepository.upsert(PlacesElementsSeparatorQuestion, dataForm).map { _ =>
          Redirect(controllers.gedcom.routes.ChoosePlacePaddingController.showForm)
        }
      }

      val formValidationResult = PlacesElementsSeparatorForm.form.bindFromRequest()
      formValidationResult.fold(errorFunction, successFunction)
  }

}
